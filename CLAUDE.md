...
- **Desktop** — a simulated OS in `src/components/desktop/`. `DesktopOS` wraps everything in `WindowManagerProvider`, renders draggable desktop icons, a `WindowLayer`, and a `Taskbar`. Each icon opens an app from `desktop/apps/` (About, Projects, Skills, Design Lab, Experience, Contact, Terminal, Chatbot, Camera, Snake, Minesweeper) inside a `Window`; "Resume.pdf" is an external link instead of a window. The Camera app uses `getUserMedia` client-side only and must release its `MediaStream` tracks on unmount. On viewports under 768px, windows open maximized.
